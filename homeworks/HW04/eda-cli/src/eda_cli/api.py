...
import io  # ← ДОБАВЛЕН ИМПОРТ IO (исправление ошибки)
...
# ---------- НОВЫЙ ЭНДПОИНТ: /quality-flags-from-csv (HW04, п. 2.3.2) ----------


@app.post("/quality-flags-from-csv")
def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Возвращает ТОЛЬКО флаги качества, включая расширенные эвристики из HW03.
    Эндпоинт не дублирует /quality-from-csv — возвращает только flags (без quality_score, ok_for_model и т.д.).
    """
    ...
    try:
        ...
        # Возвращаем ТОЛЬКО флаги, связанные с качеством из HW03
        flags_only = {
            "too_few_rows": all_flags["too_few_rows"],
            "too_many_columns": all_flags["too_many_columns"],
            "too_many_missing": all_flags["too_many_missing"],
            "has_constant_columns": all_flags["has_constant_columns"],
            "has_high_cardinality_categoricals": all_flags["has_high_cardinality_categoricals"],
            "has_suspicious_id_duplicates": all_flags["has_suspicious_id_duplicates"],
            "has_many_zero_values": all_flags["has_many_zero_values"],
        }
        return {"flags": flags_only}
